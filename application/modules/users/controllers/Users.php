...
class Users extends CI_Controller
{
    ...
    public function donasi()
    {
        $data	= [
            ...
            'hassert'	=> $this->Users_model->hassert()->result(),
            'usersl'	=> $this->Users_model->view()->result_array(),
            'donasiku'  => $this->Users_model->viewDonasi()->result_array(),
            'donasi'    => true,
            'breadcumb'	=> "Donasi untuk kebaikkan",
            'view'		=> "v_donasi"
        ];
        $this->load->view("index", $data);
    }
    
    public function addDonation($id = 0)
    {
        if ($id != 0) {
            ...
            if ($foto = '') {
                ...
                $config['upload_path'] 		= './frontend/assets/images/donasi';
                $config['allowed_types'] 	= 'jpg|png|jpeg';
                $config['max_size'] 		= 2048;
                $config['file_name'] 		= 'Donasi - ' . date('Y-m-d');
