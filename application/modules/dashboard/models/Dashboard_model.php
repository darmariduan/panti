...
class Dashboard_model extends CI_Model
{
    ...
    {
        $anakAsuh = $this->db->get('tbl_anakasuh');
        if ($anakAsuh->num_rows() > 0) {
            return $anakAsuh->num_rows();
        } else {
            return 0;
        }
    }
    ...
    public function update_prof($id, $input)
    {
        ...
        return $this->db->where('id', $id)->update('users', $input);
    }
    ...
    {
        return $this->db->insert('role', $input);
    }

    // Update Role
    public function update_role($input)
    {
        return $this->db->where('id', $input['id'])->update('role', $input);
    }
    ...
    // View Status Donatur
    public function viewStatDon($id)
    {
        $this->db->select('*');
        $this->db->from('tbl_donasi');
        $this->db->where('id', $id);
    ...
    public function update_statDon($id, $update)
    {
        return $this->db->where('id', $id)->update('tbl_donasi', $update);
    }
}
